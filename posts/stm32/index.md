...
DMA和CPU对同一地址进行访问时,仲裁器会优先选择DMA进行访问,但是也会为CPU保留一半的总线带宽(为了防止DMA数据丢失)  
Flash存储编译后的C代码和常量数据  
...
HEX数据包接收运用了状态机的思想，通过不同的S的值进中断后的行为不同，进而执行相应的动作
## I2C
I2C(inter IC Bus)  
...
## SPI
SPI(serial peripheral interface)，应用领域与I2C类似，都用来负责主控与外挂芯片之间的通信，支持一主多从但不支持多主机。通信模式为同步，全双工。与I2C区别在于，I2C是耗费硬件资源最少，SPI实现功能最多.  
