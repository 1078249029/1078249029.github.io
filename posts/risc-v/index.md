...
elf文件大致内容如下
![2025-01-01_21-57](/vx_images/78955821256435.png)
...
![](/vx_images/460211113269271.png)

![](/vx_images/69745313265826.png)

立即数
![](/vx_images/167202814261580.png)

riscv没有subi指令，这是由于subi可以用addi代替

![](/vx_images/563603014279460.png)

![](/vx_images/564253514277064.png)

直接加载一个大数会出现符号问题
![](/vx_images/19844314274566.png)
这时会出现addi x1,x1,0xfff实际上加的是-1的情况

![](/vx_images/133624314255807.png)
![](/vx_images/232894314278247.png)
代码中常用li指令赋值，之后由汇编器进行判断对指令转换成何种形式

![](/vx_images/2505814273383.png)
auipc用于构造相对地址，常用于相对寻址和位置无关码的生成中

![](/vx_images/127620115266929.png)
使用la伪指令后，链接器会自动找到label的地址并替换，最后使用auipc进行寻址

### 逻辑运算指令  

riscv只提供了与，或，异或等指令，并没有直接提供非指令，非指令通过伪指令来提供，实际上它是使用异或来实现的
![](/vx_images/426421215260063.png)
> 如下异或运算
>    10101010
> ^11111111
>    01010101
...
![](/vx_images/263142115250593.png)

算数移位只有右移没有左移，因为左移会把符号位移丢弃
![](/vx_images/64892915253097.png)
...
![](/vx_images/95633715262044.png)

![](/vx_images/9334415255538.png)
内存写指令没有0扩展或者符号扩展是因为磁盘存储单位是字节，不必区分两种扩展

### 条件分支指令  

![](/vx_images/552010419282493.png)

### 无条件跳转指令

![](/vx_images/481143220264706.png)

## 汇编函数调用约定  

**注意：ra存放的是函数返回地址，a0存放的是函数参数和返回值**
![](/vx_images/528351521254004.png)
![](/vx_images/546284815250149.png)
函数参数使用a0-a7传递，返回值使用a0和a1传递

我们在编程时常使用伪指令，尤其是call和ret
![](/vx_images/73404121250255.png)
...
![](/vx_images/190901916256656.png)

![](/vx_images/316801916276822.png)
...
* 全相联映射
* 组相联映射
![](/vx_images/594942816261800.png)

![](/vx_images/144242916279680.png)

![](/vx_images/345392916277284.png)
...
![](/vx_images/48553016274786.png)

cpu使用地址访问高速缓存时，传入的地址是虚拟地址还是物理地址？这实际上分为3种情况
1. cpu通过物理地址查询高速缓存，这种方式每次都需要查询tlb，效率较低
![](/vx_images/18493416256027.png)
2. cpu通过虚拟地址查询高速缓存
![](/vx_images/290443516278467.png)
...
```
![](/vx_images/490231512262705.png)

![](/vx_images/28291612267744.png)
...
* 新增vs和vu模式，给vm内部划分权限
![](/vx_images/68463512272049.png)

在虚拟化场景下，新增了vmm，它被允许运行在hs模式。而vm的os运行在vs模式，vm的应用程序运行在vu模式
![](/vx_images/219363512265094.png)
...
虚拟机两阶段的地址映射过程
![](/vx_images/338443912261861.png)

包括宿主机的映射过程
![](/vx_images/458113912252192.png)
