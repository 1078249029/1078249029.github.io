...
elf文件大致内容如下
![2025-01-01_21-57](assets/78955821256435.png)
...
![](assets/460211113269271.png)

![](assets/69745313265826.png)

立即数
![](assets/167202814261580.png)

riscv没有subi指令，这是由于subi可以用addi代替

![](assets/563603014279460.png)

![](assets/564253514277064.png)

直接加载一个大数会出现符号问题
![](assets/19844314274566.png)
这时会出现addi x1,x1,0xfff实际上加的是-1的情况

![](assets/133624314255807.png)
![](assets/232894314278247.png)
代码中常用li指令赋值，之后由汇编器进行判断对指令转换成何种形式

![](assets/2505814273383.png)
auipc用于构造相对地址，常用于相对寻址和位置无关码的生成中

![](assets/127620115266929.png)
使用la伪指令后，链接器会自动找到label的地址并替换，最后使用auipc进行寻址

### 逻辑运算指令  

riscv只提供了与，或，异或等指令，并没有直接提供非指令，非指令通过伪指令来提供，实际上它是使用异或来实现的
![](assets/426421215260063.png)
> 如下异或运算
>    10101010
> ^11111111
>    01010101
...
![](assets/263142115250593.png)

算数移位只有右移没有左移，因为左移会把符号位移丢弃
![](assets/64892915253097.png)
...
![](assets/95633715262044.png)

![](assets/9334415255538.png)
内存写指令没有0扩展或者符号扩展是因为磁盘存储单位是字节，不必区分两种扩展

### 条件分支指令  

![](assets/552010419282493.png)

### 无条件跳转指令

![](assets/481143220264706.png)

## 汇编函数调用约定  

**注意：ra存放的是函数返回地址，a0存放的是函数参数和返回值**
![](assets/528351521254004.png)
![](assets/546284815250149.png)
函数参数使用a0-a7传递，返回值使用a0和a1传递

我们在编程时常使用伪指令，尤其是call和ret
![](assets/73404121250255.png)
...
![](assets/190901916256656.png)

![](assets/316801916276822.png)
...
* 全相联映射
* 组相联映射
![](assets/594942816261800.png)

![](assets/144242916279680.png)

![](assets/345392916277284.png)
...
```
![](assets/490231512262705.png)

![](assets/28291612267744.png)
...
虚拟机两阶段的地址映射过程
![](assets/338443912261861.png)

包括宿主机的映射过程
![](assets/458113912252192.png)
