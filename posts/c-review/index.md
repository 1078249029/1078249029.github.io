...
![2023-09-19_08-38](assets/581673908249386.png)
![2023-09-19_08-39](assets/90294008237253.png)
\"   代表 "   \\ 代表 \    
/*    */代表多行注释
<>表示导入系统文件  ""表示导入自定义文件`
Tab == 8个空格，或者4个空格
...
![2023-09-22_08-5949](assets/499670009249389.png)
...
![2023-09-27_09-3508](assets/342723609249394.png)
正数: 原码 == 反码 == 补码
...
![2023-09-23_16-2638](assets/54091209249391.png)
![2023-09-23_16-2611](assets/248901309257424.png)
![2023-09-23_16-5554](assets/227301209237258.png)
![2023-09-23_17-2827](assets/543921309250093.png)
![2023-09-23_17-3207](assets/48801409246648.png)
所有非 0 的值都是真值
C语言浮点型数据不能取余
单目运算符优先级高于双目运算符
int a = 10; int b = 20;  则 a && b == 1
![2023-09-25_10-4453](assets/259954510230966.png)  
...
![2023-09-27_18-0028](assets/150863710249470.png)
![2023-09-27_18-0047](assets/293363710237337.png)
![2023-10-01_12-4448](assets/58503810257503.png)
![2023-10-03_10-1645](assets/196183810250172.png)
![2023-10-03_10-2054](assets/292473810246727.png)
![2023-10-03_10-3451](assets/488983810242481.png)
 ' 字母 ' 代表字母的ASCII码值，并可以用 %c 输出其值
 while 是先判断后执行 
![2023-10-03_20-2457](assets/194092820260361.png)
...
![2023-10-07_14-4543](assets/78774614231048.png)
![2023-10-07_13-5845](assets/117464714237341.png)
定义函数默认是全局的，可以在其他文件中调用，静态函数只能在本文件中使用，即使在其他文件中声明也不能使用
即使没有声明也可以调用函数，但是在 vstudio 转到定义功能不能使用
静态函数可以和全局函数重名
malloc 开辟的是连续的堆空间
![2023-10-07_15-0913](assets/194954608231049.png)
![2023-10-07_16-4537](assets/383464808257508.png)
![2023-10-07_16-4817](assets/3774908250177.png)
![2023-10-07_16-5929](assets/252204908246732.png)
![2023-10-07_17-2152](assets/380904908242486.png)
栈由高地址向低地址增长，栈底是第一个进栈的数据，栈顶就是最后一个进栈的数据。
![2023-10-08_19-2651](assets/238732719249475.png)
同理，申请0字节的空间也会报上述错误
![2023-10-08_19-2937](assets/94153019237342.png)
下图报错原因是对 NULL 指针进行赋值操作。本质上由于68行函数开辟了一个堆空间同时创建形参p，在申请10个int大小的空间后fun08就被销毁了，同时销毁的还有形参p。相当于68行是废语句
![2023-10-08_19-5338](assets/563595319257508.png)
free 空指针不会报错
结构体成员大小按最大结构体成员的倍数取，，下图中， char name 的大小为 21( sizeof(name) ==21 )，但是偏移对齐过后为24 ( int的整数倍 )
![2023-10-09_11-3636](assets/519123611231050.png)
typedef 类型名 别名;
struct 结构体类型名 {};    struct 结构体类型名 结构体名 = {}；//注意{}中的变量用 ; 隔开
结构体指针 -> 成员    结构体变量 . 成员
联合体 union 所有数据共用一块内存空间，只能输出最后一次被赋值的成员
![2023-10-09_16-5317](assets/14900717249476.png)
文件操作中的分隔符推荐以 / 正斜杠表示，如果是反斜杠，则需要输入两次
读取文件若干个字符时不能修改文件指针，文件在读取时指针会自动向下加 1
文件默认结尾是 -1 和 EOF
![2023-10-11_13-1126](assets/7011213231052.png)
结构变量名不是地址
for函数条件用 ; 隔开
可以将枚举值赋给其他变量
...
![2023-12-03_17-2714](assets/61053117237537.png)
...
结果是10
:::
![2023-12-28_11-40](assets/65104211231269.png)
...
```
结果：
![Center](assets/407281020245927.png)
