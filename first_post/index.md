...
常函数本质上是在this前加了一个const关键字，这样this不仅不能修改指向，它所指向的值也修改不了。函数在加入const的位置也不能随意指定，因为如果在函数前加const会与函数返回值类型本身隐藏的const重复从而导致const失去意义，这样只能在函数后面加了
...
返回值是交集中最后一个迭代器的位置，当遍历时需要使用，因为迭代器后面可能还有占位0元素。如果不使用返回的迭代器就有可能把交集内的所有结果全部输出，可能包含占位0
...
当在调用链中传递左值时，若后续没有调用std::move()，那么这个值就依然为左值，若调用std::move()，也不能一定说这个值就是右值
:::
...
struct Bullet {
    ...
    void draw() {
        sprite->draw(position);  // 转发给 Sprite 让他帮忙在我的位置绘制贴图，这也是代理模式名称的由来
    }
};
```
...
为了实现单例模式我们需要使用static修饰类内部的对象，使其作用域扩大为整个文件。如果对象的定义是在头文件中，在定义对象时我们需要去掉static的同时加上inline关键字，这样才能保证整个程序中只有一份实例化对象。注意此时的inline的功能不是内联，而是要告诉编译器这个对象在全局有多次定义，但我只要生成一个对象(C++17引入)
...
我们可以发现，策略模式与模板模式很像。只不过策略模式采用的是子类继承并重写父类虚函数的方式实现对共同部分的封装，而模板模式是在类内部实现，并采用权限管理的方式来实现相应的功能。因此策略模式适合于对象需要多种行为，或行为需要经常变动的情况，模板模式则适合于对象行为比较少且固定，容易在类内部实现的方式
...
struct RedBall : Ball {
    Ball *clone() override {
        return new RedBall(*this);  // 调用 RedBall 的拷贝构造函数，如果去掉*this就会拷贝RedBall的默认值，当前类型的数据并不会保存
    }
};
...
struct RedBall : Ball {
    unique_ptr<Ball> clone() override {
        return make_unique<RedBall>(*this);  // 调用 RedBall 的拷贝构造函数，拷贝的是类对象，因此是*this，拷贝指针对象的话就使用this
    }
};
...
struct BlueBall : Ball {
    ...
    int someData;  // 如果有成员变量，也会一并被拷贝到新对象中，实现了拷贝的多态
};
```
...
struct GameObject {
    ...
    void send(Message *msg) {
        for (auto &&c: components) {
            c->handleMessage(msg);    //所有组件都会处理一遍消息，很低效
        }
    }
};
```
...
struct JumpMessage {
    ...
    void accept(MessageVisitor *visitor) override {
        visitor->visit(this);  // 会调用到 visit(JumpMessage *jm) 这个重载
    }
};
